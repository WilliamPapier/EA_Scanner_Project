...
def detect_fvg(df):
    ...
    for i in range(2, len(df)):
        # bullish FVG: gap between bar[i-2].low and bar[i].high with bar[i-1] not filling
        if df['low'].iloc[i-2] > df['high'].iloc[i] and df['low'].iloc[i-1] > df['high'].iloc[i] and df['high'].iloc[i-1] < df['low'].iloc[i-2]:
            fvgs.append({"type":"bull", "top":df['low'].iloc[i-2], "bottom":df['high'].iloc[i], "index":i})
        # bearish FVG
        if df['high'].iloc[i-2] < df['low'].iloc[i] and df['high'].iloc[i-1] < df['low'].iloc[i] and df['low'].iloc[i-1] > df['high'].iloc[i-2]:
            fvgs.append({"type":"bear", "top":df['low'].iloc[i], "bottom":df['high'].iloc[i-2], "index":i})
    return fvgs[-3:] if len(fvgs)>=3 else fvgs  # only recent ones
...
def detect_order_blocks(df):
    ...
    for i in range(5, len(df)-1):
        ...
        if df['close'].iloc[i] > df['open'].iloc[i] and df['close'].iloc[i+1] < df['low'].iloc[i]:
            obs.append({"type":"bear", "high":df['high'].iloc[i], "low":df['low'].iloc[i], "index":i})
        # bullish OB: strong up move after down candle
        if df['close'].iloc[i] < df['open'].iloc[i] and df['close'].iloc[i+1] > df['high'].iloc[i]:
            obs.append({"type":"bull", "high":df['high'].iloc[i], "low":df['low'].iloc[i], "index":i})
